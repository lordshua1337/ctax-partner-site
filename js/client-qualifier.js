...
function downloadQualifierPdf() {
  ...
  doc.appendChild(CTAX_PDF.createCover('Client Qualification Report', (r.verdict_title || r.verdict || 'Analysis'), [
    { label: 'Confidence', value: conf + '%' },
    { label: 'Generated', value: new Date().toLocaleDateString() }
  ]));

  var p1 = CTAX_PDF.createPage(true);
  CTAX_PDF.addHeader(p1, 'Client Qualification Report');
  p1.innerHTML +=
    '<div style="text-align:center;padding:20px 0 24px">' +
      '<div style="display:block;padding:12px 28px;border-radius:8px;background:' + verdictColor + ';color:#fff;font-size:18px;font-weight:700;max-width:320px;margin:0 auto">' + CTAX_PDF.esc(r.verdict_title || r.verdict || '') + '</div>' +
      '<div style="font-size:13px;color:#64748b;margin-top:8px">' + CTAX_PDF.esc(r.verdict_desc || '') + ' | Confidence: ' + conf + '%</div>' +
    '</div>' +
    '<div style="display:flex;gap:12px;margin-bottom:20px">' +
      '<div style="flex:1;background:#f0f4f8;border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px">RESOLUTION PATH</div><div style="font-size:14px;font-weight:600;color:#0A1628">' + CTAX_PDF.esc(r.resolution_path || '') + '</div></div>' +
      '<div style="flex:1;background:#f0f4f8;border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px">CASE VALUE</div><div style="font-size:14px;font-weight:600;color:#0A1628">' + CTAX_PDF.esc((r.case_value_low || '') + ' - ' + (r.case_value_high || '')) + '</div></div>' +
      '<div style="flex:1;background:#f0f4f8;border-radius:8px;padding:14px;text-align:center"><div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:4px">YOUR COMMISSION</div><div style="font-size:14px;font-weight:600;color:#0A1628">' + CTAX_PDF.esc((r.commission_low || '') + ' - ' + (r.commission_high || '')) + '</div></div>' +
    '</div>' +
