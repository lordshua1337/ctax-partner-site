...
function downloadScriptPdf() {
  ...
  // Cover
  doc.appendChild(CTAX_PDF.createCover('Referral Script Package', meta || 'Custom AI-Generated Scripts', [
    { label: 'Type', value: 'Script Package' },
    { label: 'Generated', value: new Date().toLocaleDateString() }
  ]));

  // Conversation Script
  var p1 = CTAX_PDF.createPage(true);
  CTAX_PDF.addHeader(p1, 'Referral Script Package');
  p1.innerHTML += '<h2 class="ctpdf-section-title">Conversation Script</h2>' +
  ...
  CTAX_PDF.addHeader(p2, 'Referral Script Package');
  p2.innerHTML += '<h2 class="ctpdf-section-title">Email Template</h2>' +
  ...
  CTAX_PDF.addHeader(p3, 'Referral Script Package');
  p3.innerHTML += '<h2 class="ctpdf-section-title">Objection Responses</h2>' +
  ...
  if (sbResults.followup) {
    ...
    CTAX_PDF.addHeader(p4, 'Referral Script Package');
    p4.innerHTML += '<h2 class="ctpdf-section-title">Follow-Up Message</h2>' +
      '<div style="white-space:pre-wrap;font-size:13px;line-height:1.8;color:#1a2a3a">' + CTAX_PDF.esc(sbResults.followup) + '</div>';
    CTAX_PDF.addFooter(p4);
    doc.appendChild(p4);
  }
